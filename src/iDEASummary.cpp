...
//**********************************************************************//
//             Fitting Logistic Regression: iDEA                        //
//**********************************************************************//
void Mstep(mat A, vec POST_gamma, vec &tau, vec &var_coef, mat &J, vec sigma2_e, vec beta, vec beta2, double &sigma2_beta, double &sigma2_beta2)
{
	...
	double b_0beta = 20.0;
	//========================================================
	// *update sigma2_b, which maximize the Q function
	...
	size_t nr_iter = 20; // maximum iteration of newton raphson
	while (true_iter)
	{
...
//**********************************************************************//
//              Fitting Logistic Regression: iDEA Variant                        //
//**********************************************************************//
void MstepVariant(mat A, vec POST_gamma, vec &tau, vec &var_coef, mat &J, vec sigma2_e, vec beta, vec beta2, double &sigma2_beta, double &sigma2_beta2)
{
    double a_0beta = 3.0;
    double b_0beta = 20.0;
    //========================================================
...
//**********************************************************************//
//               Log Likelihood Function: iDEA                          //
//**********************************************************************//
double logL(vec SSR, double sigma2b, vec sigma2e, vec beta, vec gamma, vec pi,double a_0beta, double b_0beta)
{
	...
	log_density -= (a_0beta + 1) * log(sigma2b) + b_0beta / sigma2b;
	return log_density;
}// end func

//**********************************************************************//
//                Log Likelihood Function: iDEA Variant                 //
//**********************************************************************//
double logLVariant(vec SSR, double sigma2b, vec sigma2e, vec beta, vec gamma, vec pi,double a_0beta, double b_0beta)
{
    // here sigma2e is a vector, input by users
    double log_density = -0.5*dot(SSR, diagmat(sigma2e)*SSR);
    log_density -= 0.5 * sum(log(sigma2e));

    log_density -= 0.5 * sum(gamma % (log(sigma2b) + beta % beta / sigma2b - 2 * log(pi)));
    log_density += sum((1 - gamma) % log(1 - pi));
    log_density -= (a_0beta + 1) * log(sigma2b) + b_0beta / sigma2b;
    return log_density;
}// end func
...
void Estep(vec y, vec sigma2_e, vec &beta, vec &POST_gamma, vec &POST_beta, vec &POST_beta2, double &sigma2_beta, double &sigma2_beta2, vec pi, double &log_l_hat, int mcmc_iter, int min_degene)
{
    ...
    // initialize gamma from bernouli distribution with probability pi
	...
	SSR.zeros();
	SSR = y - XEbeta;
	log_l_hat = logL(SSR, sigma2_beta, sigma2_e, POST_beta, POST_gamma, pi,a_0beta,b_0beta);
	beta = XEbeta;
} // *end function


//*************************************************//
//           MCMC Step: iDEA Variant               //
//*************************************************//
void EstepVariant(vec y, vec sigma2_e, vec &beta, vec &POST_gamma, vec &POST_beta, vec &POST_beta2, double &sigma2_beta, double &sigma2_beta2, vec pi, double a_0beta, double b_0beta, double &log_l_hat, int mcmc_iter, int min_degene)
{
    ...
    int s_step = ceil(0.2 * mcmc_iter);
    int w_step = mcmc_iter - s_step;
    double a_beta = sum(pi)/2.0 + a_0beta;
    double b_beta = sum( beta%beta/2.0 ) + b_0beta;
    vec mean_beta = zeros<vec>(num_gene);
    vec var_beta = zeros<vec>(num_gene);
    mean_beta = beta;
    ...
    {
        // sampling sigmabeta2
        sigma2_beta = 1.0/as_scalar( randg( 1, distr_param(a_beta, 1.0/b_beta) ) );
        // *update the beta in turn,beta from spike and slab
        // Ebeta = beta_s%gamma_s; // pair-wise multipl
        Ebeta = beta_s % gamma_s + beta_s2 % (1.0 - gamma_s); // pair-wise multipl
        if (iter >= w_step)
        ...
        for (int i = 0; i < num_gene; i++)
        {
            ...
            // *modified by sun 2018.7.26.18:32:51
            //xHx = dot(X.col(i), X.col(i) ) + 1/sigma2_beta;
            xHx = 1.0 + sigma2_e(i) / sigma2_beta;
            mean_beta(i) = xHSSR / xHx;
            //var_beta[i] = sigma2_e/xHx;
            var_beta(i) = sigma2_e(i) * sigma2_beta / (sigma2_e(i) + sigma2_beta);
            // *sampling beta with mean and variance
            beta_s(i) = as_scalar(randn(1)) * sqrt(var_beta(i)) + mean_beta(i);
            log_pi(i, 1) = mean_beta(i) * mean_beta(i) / (2 * var_beta(i)) + log(sqrt(var_beta(i))) - log(sqrt(sigma2_beta)) + log(pi(i));
            log_pi(i, 0) = log(1.0 - pi(i));
        ...
        a_beta = sum(gamma_s)/2.0 + a_0beta;
        //        //b_beta = sum(Ebeta2)/(2*sigma2_e) + b_0beta;
        //        // *modidifed by sun
        b_beta = sum((beta_s%beta_s%gamma_s)/2.0) + b_0beta;
        if(iter>=w_step){
            POST_sigma2_b += sigma2_beta;
        }
...
SEXP EMMCMCStepSummary(SEXP yin, SEXP varbetain, SEXP Ain, SEXP betain, SEXP tauin, SEXP em_iterin, SEXP mcmc_iterin, SEXP min_degenein)
{
	...
	{						  // *EM Algorithm
		...
		for (size_t iter = 0; iter < em_iter; iter++)
		{
			...
			if ((abs(E_flogL - E_flogL_old) < 1e-03) || max(abs(tau_new - tau) / abs(tau_new + tau)) < 1e-03)
			{
				break;
			}
			else
		...
		return List::create(Named("beta") = beta,
                            Named("sigma2_beta") = sigma2_beta,
                            Named("sigma2_beta2") = sigma2_beta2,
                            Named("annot_coef") = tau_new,
...
//*******************************************************************//
//              Expectation Maximazation:iDEAVariant                 //
//*******************************************************************//
...
SEXP EMMCMCStepSummaryVariant(SEXP yin, SEXP varbetain, SEXP Ain, SEXP betain, SEXP tauin, SEXP em_iterin, SEXP mcmc_iterin, SEXP min_degenein)
{
    ...
    {                          // *EM Algorithm
        ...
        for (size_t iter = 0; iter < em_iter; iter++)
        {
            ...
            // *MCMC step to estimate the gamma
            //double tstart1 = clock();
            EstepVariant(y, var_beta, beta, POST_gamma, POST_beta, POST_beta2, sigma2_beta, sigma2_beta2, prior_pi, a_0beta, b_0beta, E_ElogL, mcmc_iter, min_degene);
            //double time_mcmc = (clock() - tstart1) / (double(CLOCKS_PER_SEC));
            MstepVariant(A, POST_gamma, tau_new, tau_var, info_mat, var_beta, POST_beta, POST_beta2, sigma2_beta, sigma2_beta2);
            //double time_nr = (clock() - tstart2) / (double(CLOCKS_PER_SEC));
...
void EstepWeight(vec y, vec sigma2_e, vec weight, vec &beta, vec &POST_gamma, vec &POST_beta, vec &POST_beta2, double &sigma2_beta, double &sigma2_beta2, vec pi, double &log_l_hat, int mcmc_iter, int min_degene)
{
	double a_0beta = 3.0;
    double b_0beta = 20.0;
    size_t num_gene = sigma2_e.n_elem;
	//int num_cvt = W.n_cols;

	// *set the initial values of parameters
	// *burn in times
	size_t s_step = ceil(0.2 * mcmc_iter);
	size_t w_step = mcmc_iter - s_step;
