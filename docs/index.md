...
The row names of annotation file are required to have the same type as the row names of summary statistics file, i.e. gene symbol or transcription id etc. If not, one solution is to use `biomaRt` R package to convert the gene name to make sure they are consistent each other.
...
Second, create an `iDEA` object. We encourage the user use `num_core` if a large number of annotations is input (only Linux). 
