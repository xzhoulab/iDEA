	...
	res_all <- foreach(i=1:num_annot, .combine=rbind, .options.snow=opts) %dopar% {
		...
		if(abs(res$annot_coef[2])<5 & !is.na(res$annot_coef[2]) ){
			...
			if(class(try_test)=="try-error"){
				...
				reseach <- data.frame(gene_id=object@gene_id[i], annot_coef=res$annot_coef[2], annot_var=res$annot_var[2], annot_var_louis=louis_var[2], sigma2_b=res$sigma2_beta1)
			}# end fi
		}# end fi
	}# end louis
	close(pb)
